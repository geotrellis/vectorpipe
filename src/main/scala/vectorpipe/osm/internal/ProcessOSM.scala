...
object ProcessOSM {
  ...
  /**
    ...
    */
  def reconstructWayGeometries(
    _ways: DataFrame,
    _nodes: DataFrame,
    _nodesToWays: Option[DataFrame] = None
  ): DataFrame = {
        ...
        case ((changeset, id, version, updated), rows) =>
          ...
          val geom = nds
            ...
            .map { row =>
              ...
              case coords => {
                val coordinates = coords.map(xy => new Coordinate(xy.head.toDouble, xy.last.toDouble)).toArray
  ...
  def reconstructMultiPolygonRelationGeometries(
    _relations: DataFrame,
    geoms: DataFrame
  ): DataFrame = {
    ...
    val relationGeoms = members
        ...
        .mapGroups {
          case ((changeset, id, version, minorVersion, updated, validUntil), rows) =>
            ...
            val geom = buildMultiPolygon(id, version, updated, types, roles, geoms).orNull

            new GenericRowWithSchema(Array(changeset, id, version, minorVersion, updated, validUntil, geom),
  ...
  def reconstructRouteRelationGeometries(
    _relations: DataFrame,
    geoms: DataFrame
  ): DataFrame = {
    implicit val ss: SparkSession = _relations.sparkSession
              ...
              components.map {
                case ("", geom) =>
                  ...
                  new GenericRowWithSchema(Array(changeset, id, Map(), version, minorVersion, updated,
                    validUntil, geom), TaggedVersionedElementSchema): Row
                case (role, geom) =>
                  new GenericRowWithSchema(Array(changeset, id, Map("role" -> role), version, minorVersion,
                    updated, validUntil, geom), TaggedVersionedElementSchema): Row
              }
            case None =>
              // no geometry
